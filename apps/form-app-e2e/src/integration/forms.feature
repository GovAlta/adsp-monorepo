Feature: Form app
  ...
  # TEST DATA: regression-control-examples is created all the control examples with validation rules
  @TEST_CS-4007 @regression
  Scenario: As a form user, I can see validation errors on different rules
    Given the user deletes any existing form from "Auto Test" for "regression-control-examples"
    When an authenticated user is logged in to see "regression-control-examples" application
    Then the user views a form draft of "regression-control-examples"
    When the user clicks "Controls" task on task list page
    And the user enters "hi" in a text field labelled "Single Line textbox"
    Then the user views an error message of "must NOT have fewer than 3 characters" under a text field labelled "Single Line textbox"
    When the user enters "hello" in a text area field labelled "Multiline text area"
    Then the user views an error message of "must NOT have fewer than 10 characters" under a text area labelled "Multiline text area"
    When the user clicks Back to application overview link on the form page
    When the user clicks "Standard controls" task on task list page
    And the user enters "111-111-111" in Social insurance number control
    Then the user views an error message of "Social insurance number is invalid" under Social insurance number control
    When the user clicks Back to application overview link on the form page
    When the user clicks "Summary" task on task list page
    Then the user views an error message of "must NOT have fewer than 3 characters" under "Single Line textbox" for "Controls" on review page
    And the user views an error message of "must NOT have fewer than 10 characters" under "Multiline text area" for "Controls" on review page
    And the user views an error message of "Checkbox is required" under "Checkbox" for "Controls" on review page
    And the user views an error message of "Radio group is required" under "Radio group" for "Controls" on review page
    And the user views an error message of "Data schema enumeration is required" under "Data schema enumeration" for "Controls" on review page
    And the user views an error message of "Social insurance number is invalid" under "Social insurance number" for "Standard controls" on review page