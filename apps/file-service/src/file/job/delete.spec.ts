...
describe('Delete Job', () => {
  ...
  it('can be executed', (done) => {
    ...
    fileEntityMock.setup(instance => 
      instance.delete('./files')
    ).returns(Promise.resolve(true));

    repositoryMock.setup(instance => 
      instance.find(20, null, It.Is<FileCriteria>(c => !!c.deleted))
  ...
  it('can exclude not failed from count', (done) => {
    ...
    const fileEntityMock = new Mock<FileEntity>();
    fileEntityMock.setup(instance => 
      instance.getFilePath('./files')
    ).returns('./files/test-file');

    fileEntityMock.setup(instance => 
      instance.delete('./files')
    ).returns(
      Promise.reject(new InvalidOperationError('failed'))
    );
