...
export const createScanJob = ({
  ...
  return () => {
    ...
    return fileRepository.find(
      20, null, { scanned: false, deleted: false }
    ).then((result) => 
      Promise.all(
        result.results.map(result => 
          ...
          .then((scan) => {
            ...
            logger.warn(`Error encountered scanning file ` + 
              `${result.filename} (ID: ${result.id}): ${err}`);
      ...
      logger.info(
        `Completed file scan job; scanned ${numberScanned} and found ` + 
        `${numberInfected} infected files.`
      );
