...
export const createDeleteJob = ({
  ...
  return fileRepository.find(20, null, { deleted: true })
  .then((results) => 
    Promise.all(
      results.results.map(result => 
        result.delete(rootStoragePath)
        .catch((err) => {
          logger.warn(`Error encountered deleting file ` + 
            `${result.getFilePath(rootStoragePath)} (ID: ${result.id}): ${err}`);
