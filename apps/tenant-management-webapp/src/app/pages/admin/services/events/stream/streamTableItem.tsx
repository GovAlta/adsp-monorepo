...
const SubscriberRoles = ({ roles, publicSubscribe }: SubscriberRolesProps): JSX.Element => {
  return (
    <td>
      {publicSubscribe ? (
        <GoABadge key={`roles-public`} type="information" content={'Public'} ariaLabel={`roles-public`} />
      ) : (
        roles &&
        roles.map((role): JSX.Element => {
          return <GoABadge key={`roles-${role}`} type="information" content={role} ariaLabel={`roles-${role}`} />;
        })
      )}
    </td>
  );
