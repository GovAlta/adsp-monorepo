...
import { duplicateNameCheck, Validator } from '@lib/validation/checkInput';
...
interface AddEditFormDefinitionProps {
  open: boolean;
  isEdit: boolean;
  initialValue?: RefDefinition;
  onClose: () => void;
  onSave: (definition: RefDefinition) => void;
}
...
export const AddEditRefs = ({
  ...
  const [definition, setDefinition] = useState<RefDefinition>(initialValue);
  ...
  const { errors, validators } = useValidators('name', 'name')
  ...
  return (
    <GoAModal
      ...
      open={open}
      heading={`${isEdit ? 'Edit' : 'Add'} definition refs`}
      width="640px"
      actions={
        ...
        <>
          ...
          <GoAFormItem label="Description">
            <DescriptionItem>
              <GoATextArea
                name="description"
                value={definition.refData as string}
                width="100%"
                testId="description"
                aria-label="description"
                onKeyPress={(name, value) => {
                  setDefinition({ ...definition, refData: value });
                }}
                // eslint-disable-next-line
                onChange={() => {}}
              />
