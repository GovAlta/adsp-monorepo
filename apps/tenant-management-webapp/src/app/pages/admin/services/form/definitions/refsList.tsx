...
export interface formDefinitionTableProps {
  definitions: Record<string, RefDefinition>;
  onDelete?: (PdfTemplate) => void;
  onEdit?: (PdfTemplate) => void;
}
export const RefsTable: FunctionComponent<formDefinitionTableProps> = ({ definitions, onDelete }) => {
    ...
    <DataTable data-testid="form-definitions-table">
      ...
      <tbody>
        {Object.keys(newTemplates).map((templateName) => {
          console.log(JSON.stringify(newTemplates) + 'MnewTemplates');
          return <RefItem key={templateName} formDefinition={newTemplates[templateName]} onDelete={onDelete} />;
        })}
      </tbody>
    </DataTable>
  );
