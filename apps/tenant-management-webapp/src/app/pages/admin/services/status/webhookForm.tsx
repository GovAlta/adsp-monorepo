                        ...
                        .map((definition) => {
                          return (
                            <GoACheckbox
                              ...
                              checked={webhook.eventTypes?.map((e) => e.id).includes(`${group}:${definition.name}`)}
                              onChange={(value: string) => {
                                const eventTypes = webhook.eventTypes?.map((e) => e.id);
                                const elementLocation = eventTypes.indexOf(`${group}:${definition.name}`);
                                if (elementLocation === -1) {
                                  eventTypes.push(`${group}:${value}`);
                                } else {
