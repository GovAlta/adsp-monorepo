...
Feature: Service status
  ...
  Scenario: As an interested stakeholder, I can verify status notifications for a tenant, so that I know about service availability.
    ...
    Then the user should find the event details of "Auto Test" application status changed from "{original status}" to "{new status}" for subscriber of "auto.test@gov.abtest.ca"
