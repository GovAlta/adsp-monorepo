...
Then('the user views Calendar Events page', function () {
  ...
  When('the user clicks Cancel button in Add event modal', function () {
    calendarObj.calendarModalCancelButton().shadow().find('button').click({ force: true });
    cy.wait(1000);
  });
  ...
  When('the user enters {string}, {string}, {string}, {string} in Add event modal',
  function (name, desc, checkboxAllDay, checkboxPublic) {
    cy.viewport(1920, 1080);
    calendarObj.addEventModalNameField().shadow().find('input').clear().type(name, { delay: 100, force: true });
    calendarObj.editEventModalDescriptionField().shadow().find('.goa-textarea').clear().type(desc, { force: true });
    switch (checkboxAllDay) {
      case 'yesAllDay':
      ...
      case 'noAllDay':
        break;
      default:
        expect(checkboxAllDay).to.be.oneOf(['yes', 'no']);
    }
    switch (checkboxPublic) {
      case 'yesPublic':
        calendarObj.addEventModalPublicCheckbox().shadow().find('.goa-checkbox-container').click();
        break;
      case 'noPublic':
        break;
      default:
        expect(checkboxPublic).to.be.oneOf(['yes', 'no']);
    }
  });

  Then('the user views Event page for {string}', function (name) {
    cy.viewport(1920, 1080);
    cy.wait(1000);
    calendarObj.eventNameValue().should('have.text', name);
  });

  When('the user clicks {string} button for the event of {string}', function (button, name) {
    calendarObj.eventNameValue().should('have.text', name);
        switch (button) {
          case 'Edit':
            calendarObj.eventEditButton().shadow().find('button').click({ force: true });
            cy.wait(2000);
            break;
          case 'Delete':
            calendarObj.eventDeleteButton().shadow().find('button').click({ force: true });
            cy.wait(2000);
            break;
          default:
            expect(button).to.be.oneOf(['Edit', 'Delete']);
