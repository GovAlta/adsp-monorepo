...
Feature: Calendar
  ...
  @TEST_CS-2459 @TEST_CS-2460 @REQ_CS-1653 @REQ_CS-1675 @regression
  Scenario: Test As a tenant admin, I can view the events in a calendar, so can see what is scheduled
    ...
    # Invalid data
    When the user enters " " in name field in event modal
    Then the user views the error message of "name is required" on name in event modal
  #   # Validate data
    When the user enters "autotest-addEvent", "autotest Event desc", "yesAllDay" in Add event modal
    # When the user selects "test-calendar" calendar from calendar dropdown
    # Then the user selects start and end date
	  # Then the user views Event list on calendar events page