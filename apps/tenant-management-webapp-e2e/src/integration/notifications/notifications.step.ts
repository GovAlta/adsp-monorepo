...
// The Subject/Title/Subtitle editors sit inside a properties accordion that is collapsed by
// default (CS-5126), giving the body editor more room. Collapsed content has no layout box, so
// Monaco cannot be focused or read until the section is expanded. Idempotent - safe to call
// even when the section is already open.
function expandTemplateProperties(channelKey: string) {
  notificationsObj.eventTemplatePropertiesSummary(channelKey).then(function ($summary) {
    if ($summary.attr('aria-expanded') !== 'true') {
      cy.wrap($summary).click({ force: true });
      cy.wait(500); // accordion open animation, then Monaco re-measures via automaticLayout
    }
  });
}
...
Then('the user views the email subject {string}', function (subject) {
  expandTemplateProperties('email');
