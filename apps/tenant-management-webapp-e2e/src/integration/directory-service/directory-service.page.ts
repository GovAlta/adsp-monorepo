class DirectoryServicePage {
  directoryOverviewContent() {
    return cy.xpath('//h1[text()="Directory"]/parent::main//p');
  }

  directoryAsideItems(text, link) {
    return cy.xpath(
      `//h1[text()="Directory"]/parent::main/following-sibling::aside//h3[text()="${text}"]/following-sibling::*[contains(text(), "${link}")]`
    );
  }
