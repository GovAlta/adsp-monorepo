...
export class MongoTypeRepository implements NotificationTypeRepository {
  ...
  get(space: NotificationSpaceEntity, id: string): Promise<NotificationTypeEntity> {
    return new Promise<NotificationTypeEntity>((resolve, reject) =>
      this.model.findOne({ spaceId: space.id, _id: id }, null, { lean: true }, (err, doc) =>
        err ? reject(err) : resolve(this.fromDoc(doc, space))
      )
    );
  }
  ...
  delete(type: NotificationTypeEntity): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) =>
      this.model.findOneAndDelete({ spaceId: type.spaceId, _id: type.id }, (err, doc) =>
        err ? reject(err) : resolve(!!doc)
      )
    ).then((deleted) =>
      deleted
        ? this.subscriptionRepository.deleteSubscriptions(type.spaceId, type.id).then(() => deleted)
