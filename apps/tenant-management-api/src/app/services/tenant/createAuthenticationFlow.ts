...
const createFlowExecutions = async (realm, flowAlias, token, flowId) => {
  const executionUrl = `${process.env.KEYCLOAK_ROOT_URL}/auth/admin/realms/${realm}/authentication/flows/${flowAlias}/executions/execution`;
  const executionsUrl = `${process.env.KEYCLOAK_ROOT_URL}/auth/admin/realms/${realm}/authentication/flows/${flowAlias}/executions`;
  try {
