...
internal static class LuaExtensions
{
  private static readonly IDictionary<string, MethodInfo> _functionMethods =
    typeof(ScriptFunctions).GetMethods(BindingFlags.Instance | BindingFlags.Public).ToDictionary(method => method.Name);

  public static void RegisterFunctions(this Lua lua, ScriptFunctions target)
  {
