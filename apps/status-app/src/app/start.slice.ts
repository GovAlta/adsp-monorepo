...
export interface StartState {
  apiPublicMessage: string | null;
  apiPrivateMessage: string | null;
...
export const fetchPublicResource = createAsyncThunk('start/fetchPublicResource', async (_, _thunkAPI) => {
  const response = await fetch('/api/v1/public');
  return response.json();
});

// Redux thunk to get value from a private API; access token is a required input.
export const fetchPrivateResource = createAsyncThunk('start/fetchPrivateResource', async (token: string, _thunkAPI) => {
  const response = await fetch('/api/v1/private', { headers: { Authorization: `Bearer ${token}` } });
  return response.json();
});
...
export const startReducer = createReducer(initialStartState, (builder) => {
  builder
    ...
    .addCase(fetchPublicResource.fulfilled, (state: StartState, action: PayloadAction<ApiResourceResponse>) => {
    ...
    .addCase(fetchPrivateResource.fulfilled, (state: StartState, action: PayloadAction<ApiResourceResponse>) => {
      state.loadingStatus = 'loaded';
...
export const publicResourceSelector = createSelector(
  (state: any): StartState => state[START_FEATURE_KEY],
