...
export const createNamespaceRouter = ({ logger, eventRepository }: NamespaceRouterProps): Router => {
   ...
   *                         type: string
   */
  namespaceRouter.get('/namespaces', assertAuthenticatedHandler, (req, res, next) => {
    const user: User = req.user as User;
    const top: number = parseInt((req.query.top as string) || '10', 10);
    const after: string = req.query.after as string;
    eventRepository
      .getNamespaces(top, after)
      .then((result) =>
        res.json({
          results: result.results.filter((r) => r.canAccess(user)).map(mapNamespace),
          page: result.page,
        })
      )
      .catch((err) => next(err));
   ...
   */
  namespaceRouter.get('/namespaces/:namespace', assertAuthenticatedHandler, (req, res, next) => {
    const user: User = req.user as User;
    ...
    eventRepository
      ...
      .then((entity) => (entity ? res.json(mapNamespace(entity)) : res.sendStatus(404)))
      .catch((err) => next(err));
  });
   ...
   */
  namespaceRouter.put('/namespaces/:namespace', assertAuthenticatedHandler, (req, res, next) => {
    const user: User = req.user as User;
    const name: string = req.params.namespace;
    const update = {
      name,
    ...
    eventRepository
      .getNamespace(name)
      .then((entity) => (entity ? entity.update(user, update) : NamespaceEntity.create(user, eventRepository, update)))
      .then((entity) => res.json(mapNamespace(entity)))
      .then(() => logger.info(`Event namespace ${name} updated by user ${user.name} (ID: ${user.id}).`))
      .catch((err) => next(err));
  });
