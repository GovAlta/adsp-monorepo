...
describe('findAddress', () => {
  ...
  it('should fetch address suggestions and return the result', async () => {
    ...
    expect(axiosMock.get).toHaveBeenCalledWith(
      expect.stringContaining('http://mock-address-url/find'),
      expect.objectContaining({
    ...
    expect(axiosMock.get).toHaveBeenCalledWith(
      'http://mock-address-url/find?country=CAN&languagePreference=en&lastId=CA%7CCP&maxSuggestions=5&searchTerm=7000+113+Street',
      { headers: { Authorization: 'Bearer test-token' } }
    );
