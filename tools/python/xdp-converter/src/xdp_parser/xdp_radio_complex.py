...
class XdpRadioComplex(XdpElement):
...
def extract_radio_button_labels(subform_elem):
    ...
    while i < len(children):
        ...
        if strip_namespace(child.tag) == "field":
            has_radio = any(
                strip_namespace(cb.tag) == "checkButton" and cb.attrib.get("mark") == "circle"
...
def has_checkbox_elements(draw):
    return [cb for cb in draw.iter() if cb.tag.endswith('checkButton') and cb.attrib.get('mark') == 'circle']
