...
#     row_subform: ET.Element, max_depth: int = 1
# ) -> XdpListWithDetail:
#     name = row_subform.attrib.get("name") or "Items"
#     label = _subform_label(row_subform)

#     columns: List["XdpElement"] = []
#     for field_node in _iter_leaf_field_nodes(row_subform, max_depth=max_depth):
#         try:
#             col = extract_control(field_node)
#             if col is None:
#                 continue
...
#         maybe = extract_control(placeholder)
#         if maybe:
#             columns.append(maybe)

#     return XdpListWithDetail(name=name, label=label, columns=columns)
