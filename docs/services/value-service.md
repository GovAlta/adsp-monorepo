...
`METRIC_INTERVAL_ROLLUP_MAX_CHUNK_HOURS` caps the window of the intervals that read raw metrics — in practice just `one_minute` — and defaults to a week. It cannot cut a window below a single bucket, since an interval whose chunk is narrower than its bucket would never finish backfilling, and it does not apply to composed intervals, whose cost follows the number of rollup rows rather than the span of time they cover.
