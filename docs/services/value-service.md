...
The coarse intervals are the ones to watch. A monthly bucket cannot be built from less than a month of raw metrics, so the chunk cap is floored at one bucket and `monthly` reads roughly 31 days per run however low the cap is set. Where that does not fit in the statement timeout, raise the timeout rather than lowering the cap.
