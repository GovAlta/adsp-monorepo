...
Reads consult that coverage. A request whose window falls entirely inside it is served from the rollups. A composed interval's own rollup can never cover its current, still-open period — a month is not rolled up until it is over — so a window reaching into "now" always misses on that trailing edge; rather than aggregating the whole requested window from raw `metrics` live, the read borrows the source interval's own rollup for as much of that gap as it has already covered, and only aggregates raw `metrics` for whatever sliver neither rollup has caught up to yet. Any other gap — no coverage at all yet, or a window that also reaches further back than what is covered — still falls back to a live aggregate of the raw `metrics` table for the whole window, which is slower but always complete. Rollups can therefore be populated progressively without the API losing data in the meantime.
