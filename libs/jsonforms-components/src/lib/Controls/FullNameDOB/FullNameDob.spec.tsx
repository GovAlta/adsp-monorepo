...
describe('FullName Dob ReviewControl', () => {
  ...
  it('renders empty values when no data is provided', async () => {
    ...
    const middleNameReview = screen.queryByTestId('middleName-control-full-name-dob-review-empty');
    ...
    await (async () => {
      expect(firstNameReview).toHaveTextContent('');
      expect(middleNameReview).toBeNull();
      expect(lastNameReview).toHaveTextContent('');
      expect(dobReview).toHaveTextContent('');
    });
  });
});
