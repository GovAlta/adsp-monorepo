...
import { isValidUrl } from '../components/urlUtil';
...
import { EntryDetail } from '../../styled';
...
const FormDefinitionDetails = ({ formDefinition }: { formDefinition: FormDefinition }) => {
  ...
  const selectedCoreEvent = useSelector(
    (state: RootState) => state.calendar?.coreCalendars?.['form-intake']?.calendarEvents?.[formDefinition?.id] ?? null // fallback (optional)
  );
