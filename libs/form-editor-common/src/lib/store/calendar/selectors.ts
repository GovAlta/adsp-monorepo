...
import { ModalState } from '../ssession/models';
import { defaultCalendar } from '../scalendar/models';

export const selectCalendars = createSelector(
  (state: RootState) => ({
    ...state?.calendar?.calendars,
    ...state?.calendar?.coreCalendars,
  }),
  (calendars: CalendarObjectType) => calendars
);

export const selectCoreCalendars = createSelector(
  (state: RootState) => state?.calendar.coreCalendars,
