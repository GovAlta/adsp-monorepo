...
export const createErrorHandler = (logger: Logger): ErrorRequestHandler => (err, req, res, _next) => {
  if (err instanceof UnauthorizedError || err instanceof UnauthorizedUserError ||
    err instanceof NotFoundError || err instanceof InvalidOperationError ||
    err instanceof InvalidValueError || err instanceof AdspIdFormatError
  ) {
