...
export class EventServiceClient extends ServiceClientBase {
  constructor(logger: Logger, tokenProps: TokenRequestProps, private eventUrl: string) {
  ...
  send(event: DomainEvent): Promise<void> {
    return this.tokenProps.clientId
      ? this.getToken()
          .then((token) => {
            this.logger.debug(`Sending event ${event.namespace}:${event.name} to ${this.eventUrl}/event/v1/event...`);
              ...
              request(
                ...
                (err, res) => {
                  if (err) {
                    ...
                    reject(
                      new Error(`Error encountered during request to event service with status ${res.statusCode}`)
                    );
                  } else {
                    resolve();
                  }
